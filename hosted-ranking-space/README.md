...
3. An Authority Simulator that evaluates fixed evidence trust, replay timing, action scope, and proof state. It returns a fingerprinted `ALLOW`, `REVIEW`, or `BLOCK` receipt without a model call, provider call, or Hugging Face credit. The exact same offline policy is installable from the GitHub repository as the `faultfix` CLI for terminal and CI use.
4. A selectable, read-only public case library derived from Google's GCE networking postmortem and Cloudflare's November 2025 outage postmortem. Every displayed artifact carries its provenance, stable fingerprint, source link, and data boundary; it is public postmortem evidence, not private raw telemetry.
5. An Evidence Firewall drill that classifies evidence before a model sees it. It quarantines instruction-like untrusted content, excludes facts that postdate the replay cutoff, and records a fingerprinted policy envelope. It is a deterministic simulated security test, not a claim to have detected a live attack.
6. A scoped Action Lease: a human-reviewed containment permission is bound to one command, one resource scope, one evidence fingerprint, and a short review window. Evidence drift invalidates it instead of creating standing agent authority.
7. A live-investigator adapter and four-pack simulated evaluation suite spanning capacity, DNS, identity rotation, and adversarial evidence. Configure `HF_TOKEN` for Hugging Face Inference Provider billing; the hosted model can inspect only sanitized incident packs. Faultfix validates its structured output and independently decides whether it is allowed, reviewed, or blocked.
