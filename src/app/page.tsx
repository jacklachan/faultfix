...
export default function Home() {
  ...
  function exportReceipt() { if (!receipt) return; const text = `FAULTLINE INCIDENT RECEIPT\n${receipt.id}\n\nROOT CAUSE\n${receipt.rootCause}\n\nCONFIDENCE\n${receipt.confidence}\n\nREJECTED ALTERNATIVE\n${receipt.rejected}\n\nREGRESSION TEST\n${receipt.test}\n\nCANDIDATE PATCH\n${receipt.patch}\n`; const url = URL.createObjectURL(new Blob([text], { type: "text/plain" })); const link = document.createElement("a"); link.href = url; link.download = "faultline-incident-receipt-FL-INC-042-R42.txt"; link.click(); URL.revokeObjectURL(url); }
  return <main className={styles.workbench}>
    <header className={styles.header}><div className={styles.wordmark}><span className={styles.mark}>*</span> FAULTLINE <small>incident investigator</small></div><div className={styles.simulated}>SIMULATED INCIDENT</div><div className={styles.clock}>INCIDENT CLOCK <b>00:42:17</b></div></header>
