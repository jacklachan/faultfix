...
4. **0:48 — Scroll to `01B / TRY YOUR SCENARIO`; optionally name a non-sensitive issue; under `Requested action`, choose `Make a permanent change`; leave `Causal proof` on `Incomplete`; click `Evaluate scenario authority`.** Say: “Here I am testing a scenario safely. The label never reaches a model. I am asking for a permanent change with incomplete proof, and the policy blocks it.”
5. **1:08 — Under `Causal proof`, choose `Reproduced`; click `Evaluate scenario authority` again.** Say: “Now the proof is reproduced. Even then, Faultfix does not make the change automatically. It moves the request to human review.”
